...
//PC 410 core
//  Generated on Wed Dec  5 03:18:52 2018
//
...
void main(void) {
    ...
    { // transformModelToEyePos
        ...
        highp vec4 _eyeWApos = (_mv * _lineData.p1);
        eyeWAPos = _eyeWApos;
    }

        p1eye = vec4((cam._view * vec4(eyeWAPos.xyz, 0.0)).xyz, 1.0);
    }
    ...
    { // transformModelToEyePos
        ...
        highp mat4 _mv = obj._model;
        _mv[3].xyz -= cam._viewInverse[3].xyz;
        highp vec4 _eyeWApos = (_mv * _lineData.p2);
        eyeWAPos = _eyeWApos;
    }

        p2eye = vec4((cam._view * vec4(eyeWAPos.xyz, 0.0)).xyz, 1.0);
    }
    ...
    { // transformEyeToClipPos
        gl_Position = cam._projection * vec4(eye.xyz, 1.0);
